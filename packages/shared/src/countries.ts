/**
 * ISO 3166-1 country data + helpers shared across API and web (OB-020).
 *
 * We persist a country as its ISO alpha-2 code (e.g. `GB`) and derive both the
 * display name and the flag emoji from it, so profiles can show a small flag
 * next to a tipster without storing image assets. The same list powers the
 * dialling-code selector for phone contacts.
 *
 * The flag emoji is computed from the code (two Regional Indicator Symbols),
 * so no per-country glyph table is needed.
 */
...
/** Regional-indicator flag emoji for an ISO alpha-2 code, or '' if invalid. */
...
/** Look up a country by ISO alpha-2 code (case-insensitive), or undefined. */
export function countryByCode(code: string | null | undefined): Country | undefined {
  if (!code) return undefined;
  return BY_CODE.get(code.toUpperCase());
}

/** Display name for a code, falling back to the raw value when unknown. */
export function countryName(code: string | null | undefined): string {
  if (!code) return '';
  return countryByCode(code)?.name ?? code;
}

/** Flag + name label for a code, e.g. "🇬🇧 United Kingdom". */
